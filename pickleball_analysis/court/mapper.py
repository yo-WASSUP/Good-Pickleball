...
PICKLEBALL_COURT_WIDTH = 6.096
PICKLEBALL_COURT_LENGTH = 13.4112
PICKLEBALL_KITCHEN_DISTANCE_FROM_NET = 2.1336
PICKLEBALL_COURT_DIMS = (PICKLEBALL_COURT_WIDTH, PICKLEBALL_COURT_LENGTH)


class CourtMapper:
    def __init__(self, image_court_corners, court_dimensions=PICKLEBALL_COURT_DIMS):
    ...
    def compute_court_overlay(self):
        ...
        self.vertical_lines = [
            self._line_to_image((center_x, 0), (center_x, kitchen_top)),
            self._line_to_image((center_x, kitchen_bottom), (center_x, length)),
        ]
        self.horizontal_lines = [
            self._line_to_image((0, net_y), (width, net_y)),
            self._line_to_image((0, kitchen_top), (width, kitchen_top)),
            self._line_to_image((0, kitchen_bottom), (width, kitchen_bottom)),
        ]
...
def annotate_court(image):
    ...
    guide_lines = [
        "Click 4 pickleball court outside corners: TL, TR, BR, BL",
        "Use the outer boundary lines. ROI is generated automatically. ESC cancels.",
    ]

    print("Click 4 pickleball court outside corners in order: top-left, top-right, bottom-right, bottom-left.")
    print("ROI is generated automatically.")
    ...
    if len(corners) != 4:
        print("Court annotation incomplete; retry.")
        return None, None, None
