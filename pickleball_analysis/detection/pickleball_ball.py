...
class PickleballBallTracker:
    """Detect, filter, track, and draw pickleball positions."""
    ...
    def __init__(
        ...
        self.ball_trajectory = deque(maxlen=trajectory_length)
    ...
    def detect_ball(self, frame, conf=0.18, roi_corners=None):
        ...
        if self.show_performance_stats:
            print(f"YOLO ball inference took {time.time() - t0:.2f} sec")
    ...
    def _is_outlier(self, point):
        ...
        last_point = self.ball_trajectory[-1]
    ...
    def _predict_next_position(self):
        if len(self.ball_trajectory) < 2:
            return self.ball_trajectory[-1]

        prev_x, prev_y = self.ball_trajectory[-2]
        last_x, last_y = self.ball_trajectory[-1]
        return (last_x + (last_x - prev_x), last_y + (last_y - prev_y))

    def _append_valid_point(self, point):
        self.ball_trajectory.append(point)
    ...
    def draw_trajectory(self, frame):
        ...
        if self.show_performance_stats:
            print(f"Drawing ball trajectory took {time.time() - t0:.2f} sec")

    def handle_visualization(self, frame):
        if self.show_trajectory and self.ball_trajectory:
    ...
    def get_trajectory(self):
        return list(self.ball_trajectory)
