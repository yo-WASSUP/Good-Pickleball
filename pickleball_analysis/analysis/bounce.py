...
class BounceDetector:
    ...
    def _records_to_points(self, records):
        ...
        for record in records:
            ball = record.get("pickleball_ball") or record.get("tennis_ball") or record.get("ball") or {}
            points.append(
                TrajectoryPoint(
    ...
    def _valid_bounce_court_position(self, court):
        ...
        if not np.isfinite(x) or not np.isfinite(y):
            return False
        return (
            -self.court_margin <= x <= PICKLEBALL_COURT_WIDTH + self.court_margin
            and -self.court_margin <= y <= PICKLEBALL_COURT_LENGTH + self.court_margin
        )
