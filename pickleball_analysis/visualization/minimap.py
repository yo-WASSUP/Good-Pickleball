...
class MiniMapVisualizer:
    def __init__(self, width=210, height=420, margin=18, court_margin_x=2.0, court_margin_y=3.0,
    ...
    def _draw_court(self, frame, x, y):
        ...
        view_br = self._court_to_map(x, y, PICKLEBALL_COURT_WIDTH + self.court_margin_x, PICKLEBALL_COURT_LENGTH + self.court_margin_y)
        cv2.rectangle(frame, view_tl, view_br, (70, 90, 90), 1, cv2.LINE_AA)

        tl = self._court_to_map(x, y, 0, 0)
        tr = self._court_to_map(x, y, PICKLEBALL_COURT_WIDTH, 0)
        br = self._court_to_map(x, y, PICKLEBALL_COURT_WIDTH, PICKLEBALL_COURT_LENGTH)
        bl = self._court_to_map(x, y, 0, PICKLEBALL_COURT_LENGTH)
        ...
        center_x = PICKLEBALL_COURT_WIDTH / 2

        lines = [
            ((0, net_y), (PICKLEBALL_COURT_WIDTH, net_y)),
            ((0, kitchen_top), (PICKLEBALL_COURT_WIDTH, kitchen_top)),
            ((0, kitchen_bottom), (PICKLEBALL_COURT_WIDTH, kitchen_bottom)),
            ((center_x, 0), (center_x, kitchen_top)),
            ((center_x, kitchen_bottom), (center_x, PICKLEBALL_COURT_LENGTH)),
        ]
    ...
    def _court_to_map(self, x, y, court_x, court_y):
        ...
        view_width = PICKLEBALL_COURT_WIDTH + self.court_margin_x * 2
        view_length = PICKLEBALL_COURT_LENGTH + self.court_margin_y * 2
