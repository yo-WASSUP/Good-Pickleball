...
def load_runtime_dependencies():
    ...
    global CourtLineAutoDetector, CourtTrajectoryVisualizer, PickleballBallTracker
    ...
    yolo_config_dir = os.path.join(tempfile.gettempdir(), "good-pickleball-ultralytics")
    ...
    try:
        ...
        from .detection.pickleball_ball import PickleballBallTracker as _PickleballBallTracker
    ...
    PickleballBallTracker = _PickleballBallTracker
...
class PickleballAnalysisSystem:
    def __init__(self, video_path, show_display=True, 
        ...
        if not os.path.exists(self.ball_model_path):
            raise FileNotFoundError(
                ...
                "Provide a YOLO ball model. The copied baseline is weights/tennis-ball.pt; "
                "pass another path with --ball-model when a pickleball model is trained."
            )
        ...
        self.ball_tracker = PickleballBallTracker(
    ...
    def _write_metadata(self, fps, total_frames, video_duration, template_path, corners, roi_corners, mid_height):
        metadata = {
            ...
            "models": {
                "pickleball_ball": self.ball_model_path,
                "player_detector": self.player_detector,
    ...
    def _process_frame(self, frame, template_gray, corners, roi_corners, frame_count, out, detect_frame_count):
        ...
        if self.consecutive_non_court_frames >= self.non_court_frames_threshold and self.rally_active:
            ...
            self.ball_tracker.clear_trajectory()
            self.cached_player_detection = None
        ...
        detected_ball_position = self.ball_tracker.detect_ball(frame, roi_corners=roi_corners)
        ball_position = self.ball_tracker.update_trajectory(detected_ball_position, roi_corners)
    ...
    def _finalize_bounce_detection(self):
        ...
        self.bounce_detector.annotate_video(
            ...
            draw_processed_trajectory=self.show_ball_trajectory,
        )
        if os.path.exists(annotated_path):
            os.replace(annotated_path, self.temp_output_video_path)

    def analyze_ball(self, roi_corners, corners):
